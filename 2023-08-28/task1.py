...
vowels = {'a', 'e', 'i', 'o', 'u', 'y', 'а', 'е', 'и', 'о', 'у', 'ы', 'ю', 'я'}

def count_letters(word: str, letters: set) -> dict:
    result = {}
    for letter in word:
...
def check_rythm(text: str, verbose: bool = False) -> tuple:
    vowels_in_text = []
    words = text.split()
    vowels_in_word_sum = set()

    for word in words:
        vowels_in_word = count_letters(word, vowels)
        vowels_in_text.append(vowels_in_word)
        vowels_in_word_sum.add(sum(vowels_in_word.values()))

    rythm: bool = True if len(vowels_in_word_sum) <= 1 else False

    return (rythm) if verbose is False else (rythm, vowels_in_text)

if __name__ == '__main__':
    print(check_rythm("Пам-парам-пурум Пум-пурум-карам", True))
